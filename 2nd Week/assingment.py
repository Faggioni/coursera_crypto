...
we will only test the decryption function. In the following questions you
are given an AES key and a ciphertext (both are hex encoded ) and your
goal is to recover the plaintext and enter it in the input boxes provided below.

For an implementation of AES you may use an existing crypto library such
as PyCrypto (Python), Crypto++ (C++), or any other. While it is fine to
use the built-in AES functions, we ask that as a learning experience you
...
import cripto_functions as cf

ct_input = raw_input('Enter CipherText: ')
key = raw_input('Enter Key: ')
iv, ct = cf.separate_ct(ct_input)
k1, k = cf.separate_keys(key)
"""print iv
print type(iv)
print ct
print type(ct)
...
print k
print type(k)"""
message = cf.decrypt_block_CTC(k,iv,ct)
print message
